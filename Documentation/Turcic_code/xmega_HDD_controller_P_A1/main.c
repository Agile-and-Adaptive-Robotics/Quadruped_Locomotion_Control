...
volatile float  Kp = 5;				//Proportional Gain
...
volatile float	error_m1, ctrlOut_m1;
...
int main(void)
{
	...
	dacConst = 4095./(2*maxVoltage);		//Conversion factor for Control voltage to DAC value
	//Edited to match op-amp circuit output for 0-1V from the function generator
...
//	setPoint = .5;                   //Set the setpoint to midrange for initial testing of the controller
	...
	positionDAC = (position*4095.);	//convert arm position to DAC output
	...
	ctrlCorrection =  -.85;  // Initial value
	// Control Correction to Correct the Single ended to Bipolar circuit zero offset
	// For my circuit the output of the Single ended to Bipolar circuit equaled -0.495 volts
	// when the control input (ctrlOut = 0.0) equaled 0.
	
	ctrlOut = ctrlOut_m1 + 2.514*error - 2.51*error_m1;	// PI Control 

	error_m1   = error;
	ctrlOut_m1 = ctrlOut;
	
	
//	ctrlOut = Kp * error; //P-controller
...
void encoder_init(void)
{
	...
	//setup event system
	EVSYS.CH0MUX = EVSYS_CHMUX_PORTD_PIN0_gc;	//route PINC0 to Event channel 0
...
{
	if(&DACA == dac){
		/* Get DACAOFFCAL from byte address 0x30 */
		dac->OFFSETCAL = SP_ReadCalibrationByte(0x30);
		/* Get DACAGAINCAL from byte address 0x31 */
		dac->GAINCAL = SP_ReadCalibrationByte(0x31);
		}else {
		/* Get DACBOFFCAL from byte address 0x32 */
		dac->OFFSETCAL = SP_ReadCalibrationByte(0x32);
		/* Get DACGAINCAL from byte address 0x33 */
		dac->GAINCAL = SP_ReadCalibrationByte(0x33);
	}
}
